...
const DESCRIPTIONS = {
  ...
  q: '직선과 대각선 모든 방향으로 끝까지 이동하는 체스 최강의 기물입니다. 상대 진영에 침투해 결정타를 날릴 수 있습니다.',
  k: '모든 방향으로 1칸만 이동합니다. 킹이 잡히면 (체크메이트) 게임이 끝납니다. 항상 킹 안전을 최우선으로 생각하세요.',
};
...
const DIFFICULTIES = [
  {
    id: 'easy', label: '초급', icon: '🌱',
    desc: 'AI가 무작위로 수를 둡니다. 기물의 이동 규칙과 기본 감각을 익히기에 딱 좋은 단계입니다. 패배해도 괜찮으니 자유롭게 움직여보세요.',
  },
  {
    id: 'medium', label: '중급', icon: '⚡',
    desc: 'AI가 기물 잡기·체크를 우선하며 중앙 장악을 시도합니다. 기물 교환 원칙, 중앙 장악, 킹 보호 등 핵심 전략을 배울 수 있습니다.',
  },
  {
    id: 'hard', label: '심화', icon: '🔥',
    desc: 'AI가 미니맥스 알고리즘으로 3수 앞을 내다보며 최선의 수를 선택합니다. 전략적 사고와 수읽기 능력이 필요한 실전 수준의 도전입니다.',
  },
];
...
function getCommentary(move, game) {
  ...
  if (game.isCheck()) return `체크! AI의 ${name}이/가 킹을 위협하고 있습니다. 킹을 반드시 피하세요!`;
  if (move.flags?.includes('e')) return '앙파상(En Passant)! 폰만 사용하는 특수 잡기 기술입니다.';
  if (move.flags?.includes('k')) return 'AI가 킹사이드 캐슬링으로 킹을 안전하게 보호했습니다.';
  if (move.flags?.includes('q')) return 'AI가 퀸사이드 캐슬링으로 킹을 보호하고 룩을 활성화했습니다.';
  if (move.captured) return `AI의 ${name}이/가 당신의 ${NAMES_KO[move.captured]}을/를 잡았습니다. 기물 균형을 확인하세요.`;
  if (['d4','d5','e4','e5'].includes(move.to)) return `AI가 중앙 ${move.to}을/를 장악하는 전략적인 수를 뒀습니다.`;
  return `AI가 ${name}을/를 ${move.from} → ${move.to}으로 이동했습니다. 다음 위협을 확인하세요.`;
}

function getTip(game, totalMoves) {
  if (game.isCheck()) return '⚠️ 체크 상태입니다! 킹을 이동하거나 체크를 막아야 합니다.';
  if (totalMoves < 6)  return '💡 [오프닝] 중앙 폰(e4 또는 d4)을 먼저 전진시켜 중앙을 장악하세요.';
  if (totalMoves < 14) return '💡 [오프닝] 나이트·비숍을 전개하고, 캐슬링으로 킹을 안전하게 보호하세요.';
  if (totalMoves < 30) return '💡 [미들게임] 룩을 열린 파일에 배치하고 퀸으로 공격 라인을 형성하세요.';
  return '💡 [엔드게임] 킹을 적극적으로 활용하고 폰 프로모션을 목표로 하세요.';
}
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center gap-10 p-6">
      ...
      <div className="flex flex-col sm:flex-row gap-5 w-full max-w-2xl">
        <button onClick={() => onSelect('ai')}
          className="flex-1 bg-slate-800 hover:bg-slate-700 border-2 border-sky-500/40 hover:border-sky-400 rounded-2xl p-8 text-left transition-all group shadow-xl">
          <div className="text-5xl mb-3">🤖</div>
          <h2 className="text-2xl font-bold text-sky-400 mb-2">1인용 (AI 연습)</h2>
          <p className="text-slate-400 text-sm leading-relaxed">
            AI와 대결하며 체스를 배웁니다.<br/>
            초급·중급·심화 난이도 선택 가능<br/>
            힌트 · AI 해설 · 학습 팁 제공
          </p>
        </button>
...
function DifficultySelect({ onSelect, onBack }) {
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center gap-8 p-6">
      ...
      <button onClick={onBack} className="text-slate-500 hover:text-slate-300 text-sm transition-all">
        ← 뒤로
      </button>
    </div>
  );
...
function GameOverModal({ game, mode, onReset, onMenu }) {
  ...
  if (game.isCheckmate()) {
    if (mode === 'ai') {
      ...
      icon = '🏆';
      title = loser === 'w' ? '흑(Black) 승리!' : '백(White) 승리!';
      sub = '체크메이트';
    }
  } else {
    icon = '🤝'; title = '무승부!';
    ...
    <div className="absolute inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 rounded-2xl">
      <div className="bg-slate-800 rounded-2xl p-8 text-center border border-slate-600 shadow-2xl w-72">
        ...
        <div className="flex gap-3 justify-center">
          <button onClick={onReset} className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 rounded-xl font-bold text-sm transition-all">
            다시 하기
          </button>
          <button onClick={onMenu} className="px-5 py-2.5 bg-slate-600 hover:bg-slate-500 rounded-xl font-bold text-sm transition-all">
            메뉴로
          </button>
        </div>
      </div>
...
export default function ChessApp() {
  const [screen, setScreen]               = useState('menu'); // 'menu' | 'difficulty' | 'game'
  ...
  useEffect(() => {
    ...
    const capturedCount = moveSANs.length;
    const capturedDiff  = difficulty;
    const delay = difficulty === 'hard' ? 1400 : 900;

    aiTimer.current = setTimeout(() => {
      const aiMove = getAIMove(capturedGame, capturedDiff);
      if (aiMove) {
        const g = new Chess(capturedGame.fen());
        const result = g.move(aiMove);
        setMoveSANs(prev => [...prev, result.san]);
        setCommentary(getCommentary(result, g));
        if (!g.isGameOver()) setTip(getTip(g, capturedCount + 1));
      }
  ...
  }, [moveSANs, gameMode, screen]); // intentionally excludes aiThinking, difficulty
  ...
  function handleSquareClick(sq) {
    ...
    if (selectedSq && legalTargets.has(sq)) {
      ...
      if (result) {
        setMoveSANs(prev => [...prev, result.san]);
        if (gameMode === 'ai' && !g.isGameOver()) {
          setTip('🤖 AI가 최선의 수를 고민하고 있습니다...');
          setCommentary('');
        }
      }
  ...
  function handleHint() {
    ...
    else setTip('💡 모든 수가 비슷합니다. 안전한 수를 선택하세요!');
  }
        ...
        {board.map((row, rIdx) => row.map((piece, cIdx) => {
          ...
          return (
            <button key={sq} onClick={() => handleSquareClick(sq)}
              ...
              {isTarget && (
                <span className="absolute w-[28%] h-[28%] rounded-full bg-sky-500/70 shadow-[0_0_10px_rgba(14,165,233,0.8)] animate-pulse pointer-events-none" />
              )}
            </button>
          );
        }))}
      </div>
      {isOver && <GameOverModal game={game} mode={gameMode} onReset={handleReset} onMenu={() => setScreen('menu')} />}
    </div>
  );
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="bg-slate-800 rounded-xl p-3 border border-slate-700">
        ...
        <div className="max-h-36 lg:max-h-52 overflow-y-auto text-xs font-mono">
          {moveSANs.length === 0 ? (
            <p className="text-slate-600 text-center py-2">아직 수가 없습니다</p>
          ) : (
      ...
      <div className="flex gap-2">
        {gameMode === 'ai' && (
          <button onClick={handleHint}
            disabled={game.turn() !== 'w' || aiThinking || isOver}
        ...
        <button onClick={handleUndo}
          disabled={moveSANs.length === 0 || aiThinking || (gameMode === 'ai' && game.turn() === 'b')}
        ...
        <button onClick={handleReset}
          className="flex-1 py-3 bg-slate-700 hover:bg-slate-600 rounded-xl font-bold text-sm transition-all">
          🔄 초기화
        </button>
      </div>

      <button onClick={() => { clearTimeout(aiTimer.current); setScreen('menu'); }}
        className="py-2 text-slate-500 hover:text-slate-300 text-sm text-center transition-all">
        ← 메뉴로 돌아가기
      </button>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <header className="flex items-center justify-between px-5 py-3 border-b border-slate-800 max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-2">
          {aiThinking && (
            <span className="text-xs text-sky-400 animate-pulse">AI 생각 중...</span>
          )}
          {gameMode === 'ai' && diffInfo && (
            <span className="px-2 py-1 rounded-full text-xs font-bold bg-slate-800 text-slate-300">
              {diffInfo.icon} {diffInfo.label}
            </span>
          )}
          <span className={`px-3 py-1 rounded-full text-xs font-bold ${gameMode === 'ai' ? 'bg-sky-900 text-sky-300' : 'bg-emerald-900 text-emerald-300'}`}>
            {gameMode === 'ai' ? '🤖 AI 연습' : '👥 2인용'}
          </span>
        </div>
      </header>

      <main className="max-w-5xl mx-auto px-4 py-5 flex flex-col lg:flex-row gap-5 items-start">
        <div className="w-full lg:flex-1 max-w-[560px] mx-auto lg:mx-0">
          {boardEl}
        </div>
